...
const setPlantsConfigFile = path.resolve(__dirname, 'v4_IPFS_Set100Plants.json');
const recordTimeFile = path.resolve(__dirname, 'v4_IPFS_RecordTime.json');

const { owners } = require('./data/largePlants');
const docType='plant'
...
async function main() {
    ...
    try {
        ...
        for (let counter = nextPlantNumber - 100; counter < nextPlantNumber; counter++) {
            ...
            let cid = JSON.parse(result).cid;
            const response = await fetch(`http://localhost:8081/ipfs/${cid}`);
            const json = await response.json();
            json.owner = owners[randomOwner];

            const file = await ipfs.add(JSON.stringify(json));

            cid = file.path;

            await contract.submitTransaction('updatePlant', docType+counter, cid);
            console.log("Set a plant: " + docType + counter + "   cid:"  +  cid);
        }
        ...
        recordTime.largeUpdate = endTime - startTime;
