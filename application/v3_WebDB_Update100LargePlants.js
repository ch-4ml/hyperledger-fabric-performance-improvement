...
async function main() {
    ...
    try {
        ...
        for (let counter = nextPlantNumber - 100; counter < nextPlantNumber; counter++) {
            ...
            const p = await contract.evaluateTransaction('readPlant', docType+counter);
            console.log(p);

            await Plant.updateOne({ name: docType + counter }, { $set: { owner: owners[randomOwner] } });
            const plantData = {
                name: plant.name,
                color: plant.color,
                size: plant.size,
            ...
            console.log("Update a plant: " + docType + counter + "   hash:"  +  data);
        }
